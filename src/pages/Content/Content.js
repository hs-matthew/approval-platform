...
export default function ContentPage({ 
  ...
  // ---- AUTH (same as your Dashboard had) ----
  ...
  if (!user) {
    return (
      <div className="max-w-6xl mx-auto p-6">
        ...
        <p className="text-gray-600 mb-4">You must be signed in to view content.</p>
        <a href="/login" className="inline-block bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700">
          Go to Sign In
        </a>
      </div>
    );
  ...
  // ---- (everything below is identical to your Dashboard UI) ----
  ...
  const getTypeIcon = (type) => <FileText className="w-4 h-4" />;
  ...
  const createSafePreview = (htmlContent, maxLength = 200) => {
    ...
    try {
      ...
      console.error('Error in createSafePreview:', e);
      return 'Error loading preview';
    }
  };
  const getUserById = (id) => users.find(u => u.id === id);
  const getWorkspaceById = (id) => workspaces.find(ws => String(ws.id) === String(id));
